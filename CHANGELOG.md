...
- Update notifications now work reliably on Windows by using synchronous version check

## [0.1.5] - 2025-01-08

### Fixed

- Exclude `README.md` from `rk status` and `rk pull` commands to prevent "local file missing" errors

## [0.1.4] - 2025-01-08

### Added

- Next steps guidance after `rk init` completion, showing users how to add rules and list available options

### Changed

- Files are now placed in `.claude/rules/` subdirectory instead of `.claude/` to align with Claude's official directory structure
- Updated all documentation and messages to reference `.claude/rules/`
